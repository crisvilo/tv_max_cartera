...
function constantTimeEqual(a2, b2) {
  const aa = new TextEncoder().encode(String(a2));
  const bb = new TextEncoder().encode(String(b2));
...
async function verifyPassword(password, storedHash) {
  ...
  try {
    ...
    console.error("Error verificando contrase\xF1a:", error);
    return false;
  }
}
...
async function createJWT(payload, secret) {
  if (!secret) {
    throw new Error("JWT_SECRET no est\xE1 configurado");
  }
  const header = {
    alg: "HS256",
    typ: "JWT"
...
__name(handleDBTest, "handleDBTest");
...
__name(handleDBSchema, "handleDBSchema");
async function handleLogin(request, env) {
  try {
    ...
    if (!email || !password) {
      return errorResponse(
        "Correo y contrase\xF1a son obligatorios",
        400
      );
    }
    const sql = getDB(env);
    ...
    if (!users || users.length === 0) {
      return errorResponse(
        "Correo o contrase\xF1a incorrectos",
        401
      );
    }
    const user = users[0];
    ...
    return errorResponse(
      "Error interno al iniciar sesi\xF3n",
      500,
      error.message
    );
  }
...
async function handleMe(request, env) {
  try {
    ...
    if (!auth) {
      return errorResponse(
        "Sesi\xF3n no v\xE1lida o expirada",
        401
      );
    }
    const sql = getDB(env);
...
async function handleRegister(request, env) {
  try {
    ...
    if (auth.rol !== "administrador" && auth.rol !== "admin") {
      return errorResponse(
    ...
    if (!nombre || !apellido || !zona || !email || !password) {
      return errorResponse(
...
__name(handleToggleUser, "handleToggleUser");
...
__name(handleGetTable, "handleGetTable");
...
__name(handlePostTable, "handlePostTable");
...
__name(handlePutTable, "handlePutTable");
...
__name(handleDeleteTable, "handleDeleteTable");
async function router(request, env) {
  ...
  if (pathname === "/api/auth/test" && method === "GET") {
